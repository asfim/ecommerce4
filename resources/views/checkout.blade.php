...
@push('styles')
    <style>
        ...
        .checkout-select:focus {
            border-color: var(--signal);
            box-shadow: 0 0 0 3px rgba(44, 86, 214, .12);
            outline: none;
        }
    ...
    <div class="checkout-layout">
            ...
            <form id="checkoutForm">
                <div class="checkout-card">
                    ...
                    <div class="g-row g-2">
                        <div>
                            ...
                            <input type="text" id="checkoutName" class="checkout-input w-100"
                                placeholder="Your full name" required style="border-color: #a1a1a1 !important;">
                        </div>
                        <div>
                            <label class="field-label">Phone number</label>
                            <input type="tel" id="checkoutPhone" class="checkout-input w-100"
                                placeholder="+8801xxxxxxxxx" required style="border-color: #a1a1a1 !important;">
                        </div>
                    </div>
                    <div class="g-row" style="grid-template-columns:1fr; margin-bottom:0;">
                        <div>
                            <label class="field-label">Address</label>
                            <textarea id="checkoutAddress" class="checkout-input w-100" rows="2" placeholder="House, Road, Area, City"
                                required></textarea>
                        </div>
                    </div>
                </div>
                ...
                <div class="checkout-card">
                    ...
                    <label class="ship-option selected">
                        <div class="left">
                            <input type="radio" name="ship" value="60" checked
                                style="border-color: #a1a1a1 !important;">
                            <div>
                                <div class="t">Inside Dhaka</div>
                                <div class="s">Delivery within 1–2 business days</div>
                            </div>
                        </div>
                    ...
                    <label class="ship-option">
                        <div class="left">
                            ...
                            <div>
                                <div class="t">Outside Dhaka</div>
                                <div class="s">Delivery within 3–5 business days</div>
                            </div>
                        </div>
                ...
                <div class="checkout-card">
                    ...
                    <div class="pay-tabs">
                        ...
                        <div class="pay-tab active" data-method="cod"><i class="bi bi-cash-stack"></i> Cash on Delivery
                        </div>
                    </div>

                    <input type="hidden" id="paymentMethod" value="cod">

                    <div id="sslcommerzInfo" class="p-3 border rounded"
                        style="display:none; background:#f8f9fa; font-size:13.5px;">
                        ...
                        <p class="text-muted mb-0">You will be redirected to the secure SSL Commerz gateway to pay using
                            Cards, Mobile Banking (bKash, Nagad, Rocket) or Net Banking.</p>
                    </div>

                    <div id="codInfo" class="p-3 border rounded"
                        style="display:block; background:#f0fdf4; border-color:var(--moss) !important; font-size:13.5px; color:var(--moss);">
                        <div class="d-flex align-items-center gap-2 mb-2">
                            <i class="bi bi-info-circle-fill text-success" style="font-size: 20px;"></i>
                            <span class="fw-bold">Cash on Delivery Payment</span>
                        </div>
                        <p class="mb-0">Pay cash when your order arrives at your shipping destination. No digital card
                            details needed.</p>
                    </div>
                </div>
            </form>
        ...
        <div class="summary">
            ...
            <div class="totals">
                ...
                <div class="t-row discount text-success" id="discountRow"
                    style="display:none; color: #2e7d32 !important;"><span>Discount (<span
                            id="discountCodeLabel"></span>)</span><span class="mono" id="summaryDiscount">-৳0.00</span>
                </div>
                <div class="t-row"><span>Shipping</span><span class="mono" id="summaryShipping">৳60.00</span></div>
                <div class="t-row" style="display: none !important;"><span>Estimated tax (5%)</span><span class="mono"
                        id="summaryTax">৳0.00</span></div>
                <div class="t-row grand"><span>Total</span><span class="mono" id="summaryTotal">৳0.00</span></div>
                <button type="button" id="placeOrderBtn" class="btn-place">Place order <i
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function renderSummary() {
                if (checkoutItems.length === 0) {
                    itemsContainer.innerHTML =
                        '<div class="text-center py-4 text-muted small">No items to checkout</div>';
                ...
                checkoutItems.forEach(item => {
                    ...
                    if (item.original_price && parseFloat(item.original_price) > price) {
                        ...
                        priceHtml =
                            `<del class="text-muted small d-block">৳${origTotal.toLocaleString('en-US', {minimumFractionDigits: 2})}</del>` +
                            priceHtml;
                    }
            ...
            document.querySelectorAll('input[name="ship"]').forEach(radio => {
                radio.addEventListener('change', function() {
                    document.querySelectorAll('.ship-option').forEach(opt => opt.classList.remove(
                        'selected'));
            ...
            placeOrderBtn.addEventListener('click', function() {
                ...
                const orderItems = checkoutItems.map(item => ({
                    ...
                    original_price: item.original_price ? parseFloat(item.original_price) :
